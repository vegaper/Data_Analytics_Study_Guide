...
To calculate the critical $z$-scores in python for a two tailed confidence level (we will see later what a two tail means): 
```{python}
import numpy as np
import pandas as pd
from scipy import stats
...
# Calculate z-scores: (1 + confidence_level) / 2 gives the cumulative area
z_scores = [stats.norm.ppf((1 + cl) / 2) for cl in confidence_levels]

# Create summary DataFrame
df_z = pd.DataFrame(
    {
        "Confidence Level": [f"{cl*100:.0f}%" for cl in confidence_levels],
        "z-score (Exact)": z_scores,
        "z-score (Rounded)": np.round(z_scores, 3),
    }
)
