...
This works when you pass raw NumPy arrays directly into sm.ols Statsmodels' array interface does not include an intercept (the $y$-intercept or $c$ in $y = mx + c$) by default. You have to manually call `sm.add_constant(x)` to add a column of 1s.
...
\text{body\_mass\_g} = \beta_0 + \beta_1 \cdot \text{bill\_length\_mm}
$$
